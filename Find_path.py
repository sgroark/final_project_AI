...
class MazeGame:
    def __init__(self, root, maze, algorithms, start_locations, delivery_locations):
        ...
        if self.algorithms[0] == "A*":
            ...
            local_delivery_locations = self.search_and_sort(self.delivery_locations)
            print(local_delivery_locations)
            starting = self.agent_pos
            for pos in local_delivery_locations:
                print("Start pos:", starting)
                self.run_astar(starting, pos)
                starting = pos
                print("End pos:", pos)
        elif self.algorithms[0] == "Dijkstras":
            ...
            for pos in self.delivery_locations:
                print("IN MAZE GAME IF STATEMENT")
                print(f"Goal location: {pos}")
                self.run_dij(starting, pos)
                starting = pos
                if not self.path_found:
                    print("No path able to be found. Please check input coordinates.")
                    messagebox.showerror("Error","No path found in the maze.")
                print("BACK TO MAZE GAME IF STATEMENT = End pos: ", pos)
        else:
